<template>
    <div class="starships-list">
      <div class="content-wrapper">
        <h1 class="title">Naves Estelares de Star Wars</h1>
        ...
        <div v-else-if="error" class="error-container">
          <div class="error-message">
            {{ error }}
            <button @click="fetchStarships(currentPage)" class="retry-button">Reintentar</button>
          </div>
        </div>
        
        <div v-else class="main-content">
          <div v-if="starships.length === 0" class="no-results">
  ...
  export default {
    ...
    methods: {
      async fetchStarships(page = 1) {
        ...
        this.error = null
        try {
          const response = await fetch(`https://www.swapi.tech/api/starships?page=${page}&limit=9`)
          ...
          const starshipsWithDetails = await Promise.all(
            listData.results.map(async starship => {
              try {
                const shipResponse = await fetch(starship.url)
                if (!shipResponse.ok) throw new Error('Error al cargar detalles')
                const shipData = await shipResponse.json()
                return {
                  ...starship,
                  ...shipData.result.properties,
                  uid: starship.uid
                }
              } catch (err) {
                console.error(`Error cargando ${starship.name}:`, err)
                return { ...starship, error: true }
              }
            })
          )
          this.starships = starshipsWithDetails.filter(s => !s.error)
          
        } catch (err) {
          this.error = `Error al cargar las naves: ${err.message}`
        } finally {
          this.loading = false
        }
      },
